...
// Expand calls to the internal `__cir_device_printf` marker emitted by CIRGen
// into the real AMDGPU device-printf sequence.  This must happen on LLVM IR
// (llvm::emitAMDGPUPrintfCall needs an IRBuilder) and before the device-library
// scan below, so the generated `__ockl_*` calls are seen and the right bitcode
// libraries are linked in.
static void expandCIRDevicePrintf(llvm::Module &module) {
  llvm::Function *marker = module.getFunction("__cir_device_printf");
  if (!marker)
    return;

  // CIR records the requested printf lowering as a module flag.
...
LogicalResult SerializeGPUModuleBase::optimizeModule(llvm::Module &module,
...

  {
    ...
    if (succeeded(tm)) {
      ...
      // Hoist addrspacecast before GEP for non-flat address spaces.
      //
      // CIR lowers shared-memory array subscripts as:
      //   %gep = getelementptr ..., ptr addrspace(3) @shmem, ...
      //   %flat = addrspacecast ptr addrspace(3) %gep to ptr
      //   store ptr %flat, ptr %alloca
      //
      // The narrow-pointer fixup below sees the AS3 store and shrinks the
      // alloca from ptr (8 bytes) to ptr addrspace(3) (4 bytes), but the
      // flat store still writes 8 bytes → corruption.
      //
      // OGCG instead does:
      //   %flat_base = addrspacecast ptr addrspace(3) @shmem to ptr
      //   %gep = getelementptr ..., ptr %flat_base, ...
      //   store ptr %gep, ptr %alloca
      //
      // Rewrite to match: hoist addrspacecast before GEP.
      {
        // Fix stores of narrow-AS pointers to flat-typed allocas.
        //
        // CIR lowers shared-memory pointer assignments as:
        //   %gep = getelementptr ..., ptr addrspace(3) @shmem, ...
        //   store ptr addrspace(3) %gep, ptr %flat_alloca_cast
        //
        // The alloca holds ptr (flat, 8 bytes) but the stored value is
        // ptr addrspace(3) (4 bytes) — a size mismatch that corrupts
        // memory.  OGCG avoids this by addrspacecasting shared-memory
        // pointers to flat before storing.
        //
        // Fix: insert addrspacecast narrow→flat before such stores.
        {
          unsigned fixed = 0;
          ...
          for (llvm::Function &F : module) {
            for (llvm::BasicBlock &BB : F) {
              for (auto &I : BB) {
                ...
                if (AS == 0)
                  continue;
                // Only fix if the stored pointer is narrower than flat.
          ...
          if (std::getenv("CIR_DUMP_GPU_LLVM"))
            llvm::errs() << "[CIR] Fixed " << fixed
                         << " narrow-ptr stores to flat pointers "
                         << "(addrspacecast hoist)\n";
        }
      }
  ...
  // Note: an earlier version inserted inline asm barriers after
  // workgroup.id.{x,y,z} calls to prevent AMDGPUAttributor from incorrectly
  // inferring amdgpu-no-workgroup-id-*. This was removed because the "=v"
  // constraint forced the workgroup ID into a VGPR, breaking the AMDGPU
  // uniformity analysis and causing all derived loads to use global_load
  // (VMEM) instead of s_load (SMEM). The attribute stripping below (after
  // the O3 pipeline) is the correct fix for the AMDGPUAttributor issue.

  // Attach !amdgpu.no.fine.grained.memory and !amdgpu.no.remote.memory
  // metadata to all atomicrmw instructions.  OGCG does this in
  // AMDGPUTargetCodeGenInfo::postProcessAtomicInst(); CIR has no equivalent.
  // Without this metadata, the AMDGPU backend falls back to CAS loops for
  // FP atomics (especially fp64 global_atomic_add_f64).  HIP device memory
  // (hipMalloc) is coarse-grained by default, so this is safe.
  {
    llvm::MDNode *Empty = llvm::MDNode::get(module.getContext(), {});
    for (llvm::Function &F : module) {
      for (llvm::BasicBlock &BB : F) {
        for (llvm::Instruction &I : BB) {
          auto *RMW = llvm::dyn_cast<llvm::AtomicRMWInst>(&I);
          if (!RMW)
            continue;
          if (!RMW->getMetadata("amdgpu.no.fine.grained.memory"))
            RMW->setMetadata("amdgpu.no.fine.grained.memory", Empty);
          if (!RMW->getMetadata("amdgpu.no.remote.memory"))
            RMW->setMetadata("amdgpu.no.remote.memory", Empty);
          // gfx90a's global_atomic_add_f32 flushes denormals.  Without this
          // metadata the backend falls back to a CAS loop.  OGCG adds it
          // unconditionally for __builtin_amdgcn_global_atomic_fadd_f32 and
          // conditionally (via -munsafe-fp-atomics) for generic atomics.
          // We add it for all f32 fadd atomics — matching OGCG's builtin path.
          if (RMW->getOperation() == llvm::AtomicRMWInst::FAdd &&
              RMW->getType()->isFloatTy() &&
              !RMW->getMetadata("amdgpu.ignore.denormal.mode"))
  ...
  // Set target-cpu and target-features on all functions.  OGCG sets these
  // per-function via CGCall.cpp; CIR's gpu.module lowering doesn't.  Without
  // target-cpu, the AMDGPU backend falls back to a generic subtarget and
  // misses architecture-specific instructions (packed FP32, etc.).
  // Also propagate fast-math flags from kernel functions to non-kernel device
  // helpers.  CIR's FMF walk sets flags inside the gpu.module, but helper
  // functions defined in the outer module retain default FMF.  When O3
  // inlines them, the mismatched FMF blocks vectorization/reassociation.
  {
    StringRef chip = target.getChip();
    StringRef features = target.getFeatures();

    llvm::FastMathFlags kernelFMF;
    bool foundKernelFMF = false;

    for (llvm::Function &F : module) {
      if (F.isDeclaration())
        continue;
      if (!chip.empty() && !F.hasFnAttribute("target-cpu"))
        F.addFnAttr("target-cpu", chip);
      ...
      if (!foundKernelFMF &&
        ...
        for (llvm::BasicBlock &BB : F) {
          for (llvm::Instruction &I : BB) {
            if (auto *FPI = llvm::dyn_cast<llvm::FPMathOperator>(&I)) {
              if (FPI->getFastMathFlags().any()) {
                kernelFMF = FPI->getFastMathFlags();
                foundKernelFMF = true;
                break;
              }
            }
    ...
    if (foundKernelFMF && kernelFMF.any()) {
      for (llvm::Function &F : module) {
        if (F.isDeclaration() ||
            F.getCallingConv() == llvm::CallingConv::AMDGPU_KERNEL)
          continue;
        for (llvm::BasicBlock &BB : F) {
          for (llvm::Instruction &I : BB) {
            if (llvm::isa<llvm::FPMathOperator>(I))
              I.setFastMathFlags(kernelFMF);
          }
        }
      }
    }

    // When ninf+nnan are active, fold llvm.is.fpclass calls that test for
    // infinity or NaN classes to constant false.  CIR emits is.fpclass for
    // isinf/isnan builtins; OGCG uses nofpclass(nan inf) on call args to
    // fold them.  is.fpclass doesn't carry FMF, so LLVM's InstCombine can't
    // fold it based on the kernel's FMF alone — we do it explicitly.
    if (kernelFMF.noInfs() || kernelFMF.noNaNs()) {
      llvm::SmallVector<llvm::CallInst *, 8> toFold;
      for (llvm::Function &F : module) {
        for (llvm::BasicBlock &BB : F) {
          for (llvm::Instruction &I : BB) {
            ...
            if (!Callee ||
                Callee->getIntrinsicID() != llvm::Intrinsic::is_fpclass)
              continue;
            auto *Mask = llvm::dyn_cast<llvm::ConstantInt>(CI->getArgOperand(1));
            if (!Mask)
              continue;
            unsigned test = Mask->getZExtValue();
            // fcInf = 0x204 = 516, fcNan = 0x003 = 3
            bool testsInf = (test & 0x204) != 0;
            bool testsNan = (test & 0x003) != 0;
            bool testsOther = (test & ~0x207u) != 0;
            if (testsOther)
              continue;
            if ((testsInf && kernelFMF.noInfs()) &&
                (!testsNan || kernelFMF.noNaNs()))
              toFold.push_back(CI);
            else if ((testsNan && kernelFMF.noNaNs()) && !testsInf)
              toFold.push_back(CI);
          }
        }
      }
      for (llvm::CallInst *CI : toFold) {
        CI->replaceAllUsesWith(
            llvm::ConstantInt::getFalse(CI->getType()));
        CI->eraseFromParent();
      }
    }
  }

  // Emit llvm.assume(blockDim.x * gridDim.x >= arg[i]) for kernels with
  // the "grid-covers-args" attribute (set by OffloadPropagateGridCoverage).
  // Uses implicitarg_ptr loads (hidden_group_size_x at offset 12, i16;
  // hidden_block_count_x at offset 0, i32) to match the exact SSA values
  // the grid-stride loop uses for its stride computation.  CSE/GVN during
  // O3 will unify these with the loop's values, letting SCEV prove the
  // trip count is <= 1 and enabling LoopFullUnroll to eliminate the loop.
  {
    llvm::Function *assumeDecl = llvm::Intrinsic::getOrInsertDeclaration(
        &module, llvm::Intrinsic::assume);
    llvm::Function *implicitArgDecl =
        llvm::Intrinsic::getOrInsertDeclaration(
            &module, llvm::Intrinsic::amdgcn_implicitarg_ptr);

    for (llvm::Function &F : module) {
      if (F.getCallingConv() != llvm::CallingConv::AMDGPU_KERNEL)
        continue;
      if (!F.hasFnAttribute("grid-covers-args"))
        continue;

      llvm::StringRef val =
          F.getFnAttribute("grid-covers-args").getValueAsString();

      // Parse comma-separated arg indices.
      llvm::SmallVector<unsigned, 4> coveredArgs;
      while (!val.empty()) {
        llvm::StringRef tok;
        std::tie(tok, val) = val.split(',');
        unsigned idx;
        if (!tok.getAsInteger(10, idx))
          coveredArgs.push_back(idx);
      }
      ...
      llvm::Value *iap = builder.CreateCall(implicitArgDecl);

      // Load hidden_group_size_x (offset 12, i16) = blockDim.x
      llvm::Type *i8Ty = builder.getInt8Ty();
      llvm::Type *i16Ty = builder.getInt16Ty();
      llvm::Type *i32Ty = builder.getInt32Ty();
      llvm::Value *gep12 = builder.CreateConstInBoundsGEP1_32(i8Ty, iap, 12);
      llvm::LoadInst *groupSize = builder.CreateLoad(i16Ty, gep12);
      groupSize->setMetadata(llvm::LLVMContext::MD_invariant_load,
                             llvm::MDNode::get(ctx, {}));
      llvm::Value *groupSize32 = builder.CreateZExt(groupSize, i32Ty);

      // Load hidden_block_count_x (offset 0, i32) = gridDim.x
      llvm::LoadInst *blockCount = builder.CreateLoad(i32Ty, iap);
      blockCount->setMetadata(llvm::LLVMContext::MD_invariant_load,
                              llvm::MDNode::get(ctx, {}));

      // stride = blockDim.x * gridDim.x
      llvm::Value *stride = builder.CreateMul(groupSize32, blockCount);

      // Emit assumes on blockCount (SCEVUnknown) rather than on stride
      // (SCEVMulExpr).  SCEV's applyLoopGuards can only rewrite
      // SCEVUnknown nodes, so assumes on the product don't narrow the
      // stride range.  By constraining blockCount directly:
      //   assume(blockCount > 0)  → GuardedStride = 256 * smax(1, bc) > 0
      //   assume(N <=u stride)    → GuardedRHS = umin(N, 256*bc) <= stride
      llvm::Value *zero = llvm::ConstantInt::get(i32Ty, 0);

      // Assume stride > 0 on the product value itself.  blockDim.x >= 1
      // and gridDim.x >= 1 for any real GPU launch.
      llvm::Value *stridePos = builder.CreateICmpSGT(stride, zero);
      builder.CreateCall(assumeDecl, stridePos);

      for (unsigned argIdx : coveredArgs) {
        if (argIdx >= F.arg_size())
          continue;
        llvm::Argument *arg = F.getArg(argIdx);
        llvm::Value *argVal = arg;

        // Extend/truncate to i32 if needed.
        if (arg->getType()->isIntegerTy() &&
            arg->getType()->getIntegerBitWidth() != 32) {
          if (arg->getType()->getIntegerBitWidth() < 32)
            argVal = builder.CreateZExt(arg, i32Ty);
          else
            argVal = builder.CreateTrunc(arg, i32Ty);
        } else if (!arg->getType()->isIntegerTy()) {
          continue;
        }

        // Emit assume(N <=u stride) so SCEV's howManyLessThans can
        // prove the backedge is taken at most 0 times.
        llvm::Value *cmp = builder.CreateICmpULE(argVal, stride);
        builder.CreateCall(assumeDecl, cmp);

        // Emit assume(N >=s 0) — the grid-covered arg is a count/size
        // that's non-negative. This helps SCEV when the kernel uses
        // signed comparisons (icmp slt) for bounds checks.
        llvm::Value *nonneg = builder.CreateICmpSGE(argVal, zero);
        builder.CreateCall(assumeDecl, nonneg);
      }

      F.removeFnAttr("grid-covers-args");
    }
  }

  // Emit llvm.assume(gridDim.<d> * divisor >= arg[i]) for kernels carrying
  // "grid-covers-dims", a comma-separated list of dim:arg:divisor triples.
  //
  // This is the counterpart of "grid-covers-args" above for loops strided
  // over the Y or Z dimension, whose stride is the grid extent alone rather
  // than grid extent times block size.  The grid extent is read from the same
  // implicitarg block the backend lowers gridDim to, so CSE unifies it with
  // whatever the loop already computed.
  {
    llvm::Function *assumeDecl = llvm::Intrinsic::getOrInsertDeclaration(
        &module, llvm::Intrinsic::assume);
    llvm::Function *implicitArgDecl =
    ...
    for (llvm::Function &F : module) {
      if (F.getCallingConv() != llvm::CallingConv::AMDGPU_KERNEL)
        continue;
      if (!F.hasFnAttribute("grid-covers-dims"))
        continue;

      llvm::StringRef val =
          F.getFnAttribute("grid-covers-dims").getValueAsString();
      ...
      llvm::Type *i32Ty = builder.getInt32Ty();
      llvm::Value *zero = llvm::ConstantInt::get(i32Ty, 0);
      llvm::Value *iap = nullptr;

      while (!val.empty()) {
        llvm::StringRef tok;
        std::tie(tok, val) = val.split(',');
        llvm::StringRef dimStr, argStr, divStr;
        std::tie(dimStr, tok) = tok.split(':');
        std::tie(argStr, divStr) = tok.split(':');
        unsigned dim, argIdx;
        uint64_t divisor;
        if (dimStr.getAsInteger(10, dim) || argStr.getAsInteger(10, argIdx) ||
            divStr.getAsInteger(10, divisor))
          continue;
        // x is handled by "grid-covers-args"; divisor 0 would be nonsense.
        if (dim < 1 || dim > 2 || divisor == 0 || argIdx >= F.arg_size())
          continue;
        ...
        if (!iap)
          iap = builder.CreateCall(implicitArgDecl);

        // hidden_block_count_{x,y,z} live at offsets 0, 4, 8 as i32.
        llvm::Value *gep = builder.CreateConstInBoundsGEP1_32(
            i8Ty, iap, static_cast<unsigned>(dim * 4));
        llvm::LoadInst *blockCount = builder.CreateLoad(i32Ty, gep);
        blockCount->setMetadata(llvm::LLVMContext::MD_invariant_load,
                                llvm::MDNode::get(ctx, {}));
        ...
        // Constrain the grid extent itself rather than the product: SCEV's
        // applyLoopGuards only rewrites SCEVUnknown nodes, so a guard on a
        // multiply would not narrow anything.
        builder.CreateCall(assumeDecl,
                           builder.CreateICmpSGT(blockCount, zero));

        llvm::Value *bound = blockCount;
        if (divisor != 1)
          bound = builder.CreateMul(
              blockCount, llvm::ConstantInt::get(i32Ty, divisor));

        builder.CreateCall(assumeDecl, builder.CreateICmpULE(argVal, bound));
        builder.CreateCall(assumeDecl, builder.CreateICmpSGE(argVal, zero));
      }

      F.removeFnAttr("grid-covers-dims");
    }
  }

  // Emit llvm.assume(countArg >= 0) for kernels with "buffer-size-args".
  // The attribute encodes pairs of (ptr_arg_index, count_arg_index) where
  // the count arg controls the number of elements the pointer arg accesses.
  // Proving non-negativity helps SCEV compute trip counts for loops bounded
  // by the count arg.
  {
    llvm::Function *assumeDecl = llvm::Intrinsic::getOrInsertDeclaration(
        &module, llvm::Intrinsic::assume);
    ...
    for (llvm::Function &F : module) {
      if (F.getCallingConv() != llvm::CallingConv::AMDGPU_KERNEL)
        continue;
      if (!F.hasFnAttribute("buffer-size-args"))
        continue;

      llvm::StringRef val =
          F.getFnAttribute("buffer-size-args").getValueAsString();

      // Parse comma-separated pairs: "ptrIdx,countIdx,ptrIdx,countIdx,..."
      ...
      for (unsigned k = 0; k + 1 < indices.size(); k += 2) {
        unsigned countIdx = indices[k + 1];
        if (countIdx >= F.arg_size())
          continue;
        llvm::Argument *countArg = F.getArg(countIdx);
        if (!countArg->getType()->isIntegerTy())
          continue;

        llvm::Value *countVal = countArg;
        if (countArg->getType()->getIntegerBitWidth() != 32) {
          if (countArg->getType()->getIntegerBitWidth() < 32)
            countVal = builder.CreateZExt(countArg, i32Ty);
          else
            countVal = builder.CreateTrunc(countArg, i32Ty);
        }

        llvm::Value *nonneg = builder.CreateICmpSGE(countVal, zero);
        builder.CreateCall(assumeDecl, nonneg);
      }

      F.removeFnAttr("buffer-size-args");
    }
  }

  // Add function attributes that OGCG's codegen emits but CIR's doesn't:
  //   - mustprogress: all C++11 functions (enables LICM, DSE, loop opts)
  //   - norecurse: AMDGPU kernels (enables interprocedural opts)
  // Note: do NOT set memory(argmem: readwrite) on non-kernel helpers.
  // While it correctly describes most helpers, it causes InstCombine to
  // eliminate atomic operations (atomicAdd/atomicMax) whose writes are
  // only observed by other GPU threads, not by the calling kernel.
  // OGCG leaves the default memory(readwrite), which is safe.
  if (optLevel > 0) {
    for (llvm::Function &F : module) {
      if (F.isDeclaration())
        continue;
  ...
  // Pre-O3 cleanup: promote scalar allocas to SSA values.
  //
  // CIR's LLVM IR emission produces allocas for every scalar local variable
  // (unlike OGCG which emits SSA directly).  These small allocas have GEPs
  // in private address space that trigger AMDGPU's unroll threshold boost
  // from 300 to 2700.  Running SROA+InstCombine before O3 eliminates the
  // small allocas so that when the CGSCC inliner brings helper functions
  // into kernels, only SSA values arrive — matching OGCG's IR shape and
  // preventing spurious over-unrolling.
  if (optLevel > 0) {
    FailureOr<llvm::TargetMachine *> preTM = getOrCreateTargetMachine();

    llvm::LoopAnalysisManager preLam;
    llvm::FunctionAnalysisManager preFam;
    llvm::CGSCCAnalysisManager preCgam;
    llvm::ModuleAnalysisManager preMam;

    llvm::PassBuilder prePb(succeeded(preTM) ? *preTM : nullptr);
    if (succeeded(preTM))
      (*preTM)->registerPassBuilderCallbacks(prePb);
    prePb.registerModuleAnalyses(preMam);
    prePb.registerCGSCCAnalyses(preCgam);
    prePb.registerFunctionAnalyses(preFam);
    prePb.registerLoopAnalyses(preLam);
    prePb.crossRegisterProxies(preLam, preFam, preCgam, preMam);

    llvm::ModulePassManager preMpm;
    llvm::FunctionPassManager preFpm;
    preFpm.addPass(llvm::SROAPass(llvm::SROAOptions::ModifyCFG));
    preFpm.addPass(llvm::InstCombinePass());
    preMpm.addPass(llvm::createModuleToFunctionPassAdaptor(std::move(preFpm)));
    preMpm.run(module, preMam);
  }

  // Mark stores through writeonly kernel args as nontemporal.  This runs
  // AFTER pre-O3 SROA so that store pointers trace directly to kernel
  // arguments (SROA eliminates the arg→alloca→load pattern).
  {
    auto &ctx = module.getContext();
    llvm::MDNode *ntNode = llvm::MDNode::get(
        ctx, llvm::ConstantAsMetadata::get(llvm::ConstantInt::getTrue(ctx)));

    for (llvm::Function &F : module) {
      if (F.getCallingConv() != llvm::CallingConv::AMDGPU_KERNEL)
        continue;
      if (!F.hasFnAttribute("nontemporal-store-args"))
        continue;

      llvm::StringRef val =
          F.getFnAttribute("nontemporal-store-args").getValueAsString();

      llvm::SmallDenseSet<unsigned, 4> ntArgIndices;
      while (!val.empty()) {
        llvm::StringRef tok;
        std::tie(tok, val) = val.split(',');
        unsigned idx;
        if (!tok.getAsInteger(10, idx))
          ntArgIndices.insert(idx);
      }

      if (ntArgIndices.empty())
        continue;
      ...
      for (auto &BB : F) {
        for (auto &I : BB) {
          ...
          if (auto *arg = llvm::dyn_cast<llvm::Argument>(ptr)) {
            if (ntArgIndices.count(arg->getArgNo()))
              SI->setMetadata(llvm::LLVMContext::MD_nontemporal, ntNode);
          }
        }
      }

      F.removeFnAttr("nontemporal-store-args");
    }
  }
  ...
  LogicalResult result = [&]() -> LogicalResult {
    ...
    // Let the unroller act on a *maximum* trip count, not only an exact one.
    //
    // The offload passes prove bounds the source does not state -- that a
    // grid-strided loop cannot run more than once, because the launch covers
    // the whole range -- and emit them as assumes.  SCEV consumes those and
    // derives a maximum backedge count, but LoopUnroll ignores a maximum
    // unless UpperBound is set, which no AMDGPU default enables.  Without
    // this the bound is computed and then discarded.
    //
    // Added here rather than in the target's unrolling preferences so it
    // applies to this pipeline only: the facts come from host-side knowledge
    // that a plain device compilation does not have, and there is no reason
    // to change unrolling for code compiled without them.
    if (ol != llvm::OptimizationLevel::O0) {
      llvm::LoopUnrollOptions unrollOpts(static_cast<int>(optLevel));
      unrollOpts.setUpperBound(true);
      llvm::FunctionPassManager fpm;
      fpm.addPass(llvm::RequireAnalysisPass<
    ...
    // The per-module pipeline vectorizes loops and marks them with
    // !llvm.loop.isvectorized, but the unroller in that same pipeline run
    // doesn't further unroll them.  OGCG gets a second optimization pass
    // via the LTO backend (CodeGen O3 + LTO O3).  We match that by running
    // the LTO pipeline on the already-optimized module, which catches
    // post-vectorization unrolling and other cross-function opportunities
    // exposed by linking the ROCm bitcode libraries.
    if (ol != llvm::OptimizationLevel::O0 && !std::getenv("CIR_NO_LTO")) {
      llvm::ModulePassManager ltoMpm;
      ltoMpm.addPass(pb.buildLTODefaultPipeline(llvm::OptimizationLevel::O2,
                                                /*ExportSummary=*/nullptr));
      ltoMpm.run(module, mam);
    }


    if (std::getenv("CIR_DUMP_GPU_POST_O3")) {
      std::error_code ec;
      llvm::raw_fd_ostream os("/tmp/cir_gpu_post_o3.ll", ec,
    ...
    // after heavy inlining from CIR).  Only strip the "no" attribute for
    // intrinsics the kernel actually calls, so the backend initializes
    // those registers.  Attributes for genuinely unused dimensions are
    // kept, avoiding unnecessary hardware resource setup.
    for (llvm::Function &F : module) {
      if (F.getCallingConv() != llvm::CallingConv::AMDGPU_KERNEL)
        continue;
      // Scan the kernel body for actual intrinsic uses.
      bool usesWorkgroupIdX = false, usesWorkgroupIdY = false,
           usesWorkgroupIdZ = false;
      bool usesWorkitemIdX = false, usesWorkitemIdY = false,
           usesWorkitemIdZ = false;
      bool usesDispatchPtr = false, usesImplicitargPtr = false;
      for (llvm::BasicBlock &BB : F) {
        for (llvm::Instruction &I : BB) {
          auto *CI = llvm::dyn_cast<llvm::CallInst>(&I);
          if (!CI)
          ...
          if (name == "llvm.amdgcn.workgroup.id.x")
            usesWorkgroupIdX = true;
          else if (name == "llvm.amdgcn.workgroup.id.y")
            usesWorkgroupIdY = true;
          else if (name == "llvm.amdgcn.workgroup.id.z")
            usesWorkgroupIdZ = true;
          else if (name == "llvm.amdgcn.workitem.id.x")
            usesWorkitemIdX = true;
          else if (name == "llvm.amdgcn.workitem.id.y")
            usesWorkitemIdY = true;
          else if (name == "llvm.amdgcn.workitem.id.z")
            usesWorkitemIdZ = true;
          else if (name == "llvm.amdgcn.dispatch.ptr")
            usesDispatchPtr = true;
          else if (name == "llvm.amdgcn.implicitarg.ptr")
            usesImplicitargPtr = true;
        }
      }
      // Strip "no" attributes only for intrinsics the kernel uses,
      // ensuring the hardware initializes those registers.
      if (usesWorkgroupIdX)
...
void SerializeGPUModuleBase::addControlVariables(
  ...
  if (any(libs & (AMDGCNLibraries::Ocml | AMDGCNLibraries::Ockl))) {
    ...
    // ROCm's oclc_wavefrontsize64_{on,off}.bc defines this alongside
    // __oclc_wavefrontsize64, and OCKL functions reference it (device printf
    // reaches it through __ockl_printf_append_*).  Synthesizing only the
    // former leaves an undefined variable, which makes the HSA loader reject
    // the whole code object with HSA_STATUS_ERROR_VARIABLE_UNDEFINED -- every
    // kernel in it then fails to launch with "no kernel image is available".
    addControlVariable("__oclc_wavefrontsize_log2", wave64 ? 6 : 5, 32);
    // Get the ISA version.
