...
Address CIRGenFunction::emitArrayToPointerDecay(const Expr *e,
  ...
  // Normally the lvalue's pointee is the array type itself.  For an array with
  // no statically-known bound -- e.g. an `extern __shared__` declaration whose
  // storage is supplied at launch -- the address already points at the element
  // type, so there is no array type to decay from.  maybeBuildArrayDecay()
  // below handles both shapes, so only check the invariant in the array case.
  ...
  assert((!pointeeTy || mlir::isa<cir::ArrayType>(arrayTy)) &&
         "expected array");
  assert((!pointeeTy || pointeeTy == arrayTy));
