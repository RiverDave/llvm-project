...
mlir::LogicalResult CIRGenFunction::emitIfStmt(const IfStmt &s) {
  ...
  auto ifStmtBuilder = [&]() -> mlir::LogicalResult {
    ...
    if (constantFoldsToBool(s.getCond(), condConstant, s.isConstexpr())) {
      ...
      // block. This avoids emitting dead code and simplifies the CFG
      // substantially.
...
/// Build a CIR LoopHintAttr from AST loop attributes (#pragma unroll, etc.)
static cir::LoopHintAttr
buildLoopHintAttr(mlir::MLIRContext *ctx, clang::ASTContext &astCtx,
                  ArrayRef<const Attr *> attrs) {
  cir::LoopUnrollModeAttr unrollMode;
  std::optional<int64_t> unrollCount;

  for (const auto *a : attrs) {
    if (const auto *hint = dyn_cast<clang::LoopHintAttr>(a)) {
      using LH = clang::LoopHintAttr;
      if (hint->getOption() == LH::Unroll) {
        switch (hint->getState()) {
        case LH::Enable:
          unrollMode = cir::LoopUnrollModeAttr::get(
              ctx, cir::LoopUnrollMode::Enable);
          break;
        case LH::Full:
          unrollMode = cir::LoopUnrollModeAttr::get(
              ctx, cir::LoopUnrollMode::Full);
          break;
        case LH::Disable:
          unrollMode = cir::LoopUnrollModeAttr::get(
              ctx, cir::LoopUnrollMode::Disable);
          break;
        default:
          break;
        }
      } else if (hint->getOption() == LH::UnrollCount) {
        if (auto *value = hint->getValue()) {
...
mlir::LogicalResult
CIRGenFunction::emitForStmt(const ForStmt &s, ArrayRef<const Attr *> attrs) {
  cir::ForOp forOp;
...
mlir::LogicalResult
CIRGenFunction::emitDoStmt(const DoStmt &s, ArrayRef<const Attr *> attrs) {
