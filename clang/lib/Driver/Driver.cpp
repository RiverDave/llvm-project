    ...
    getDeviceDependences(OffloadAction::DeviceDependences &DA,
      ...
      if (CompileDeviceOnly || CurPhase == phases::Backend) {
        ...
        for (unsigned I = 0, E = GpuArchList.size(); I != E; ++I) {
          ...
          FatbinArchs.push_back(GpuArchList[I].ArchName.data());
        }
...
InputInfoList Driver::BuildJobsForActionNoCache(
  ...
  if (const auto *MA = dyn_cast<CIRMergeJobAction>(JA)) {
    ...
    if (Inputs.size() >= 2) {
      InputInfos.append(BuildJobsForAction(C, Inputs[0], TC, BA,
                                           /*AtTopLevel=*/false, MultipleArchs,
                                           LinkingOutput, CachedResults,
                                           Action::OFK_None));
      auto HIPTCRange = C.getOffloadToolChains(Action::OFK_HIP);
      const ToolChain *DevTC =
          (HIPTCRange.first != HIPTCRange.second) ? HIPTCRange.first->second
                                                  : TC;
      bool multiDevice = (Inputs.size() > 2);
      for (size_t i = 1; i < Inputs.size(); ++i) {
        BoundArch DevArch = Inputs[i]->getOffloadingArch();
        InputInfos.append(BuildJobsForAction(
            C, Inputs[i], DevTC, DevArch, /*AtTopLevel=*/false,
            /*MultipleArchs=*/multiDevice, LinkingOutput, CachedResults,
            Action::OFK_HIP));
      }
    }
  } else {
