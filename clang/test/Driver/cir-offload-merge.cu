...
// NO-LAUNCH-BOUNDS: "{{.*}}cir-offload-merge{{(\.exe)?}}" "-combine" "-disable-cir-infer-launch-bounds" "-targets=host-x86_64-unknown-linux-gnu,cuda-nvptx64-nvidia-cuda-unknown-sm_80"

// RUN: %clang -### -target x86_64-unknown-linux-gnu -x cuda -fclangir \
// RUN:   --cuda-gpu-arch=sm_80 -nocudainc -nocudalib \
// RUN:   --clangir-offload-merge -fno-cir-prop-kernel-args -c %s 2>&1 \
// RUN: | FileCheck %s --check-prefix=NO-CONST-PROP

// NO-CONST-PROP: "{{.*}}cir-offload-merge{{(\.exe)?}}" "-combine" "-disable-cir-prop-kernel-args" "-targets=host-x86_64-unknown-linux-gnu,cuda-nvptx64-nvidia-cuda-unknown-sm_80"
